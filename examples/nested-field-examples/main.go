...
func main() {
	...
	// Basic aggregation demonstration
	fmt.Println("\n📈 第二部分：嵌套字段聚合")
...
// Demonstration of basic nested field access
func demonstrateBasicNestedAccess(ssql *streamsql.Streamsql) {
	// SQL queries use basic nested fields
	...
	testData := []map[string]any{
		...
		},
		{
			"device": map[string]any{
				...
				"location": "room-B", // Mismatched conditions
			},
			"sensor": map[string]any{
				"temperature": 30.0,
				"humidity":    55.8,
		...
		},
		{
			...
			"sensor": map[string]any{
				"temperature": 15.0, // Mismatched conditions
				"humidity":    65.3,
			},
			"timestamp": time.Now().Unix(),
		},
...
// Demonstration of nested field aggregation
func demonstrateNestedAggregation(ssql *streamsql.Streamsql) {
	// SQL Query: Nested field aggregation
	...
	go func() {
		for i := 0; i < 9; i++ {
			...
			ssql.Emit(data)
			time.Sleep(300 * time.Millisecond) // Transmits a data piece every 300ms
		}
	}()
