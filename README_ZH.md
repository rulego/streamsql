...
- 数据分析
  ...
  - 支持分组聚合
  - 支持过滤条件
- 复杂事件处理（CEP）——**轻量边缘引擎中独有**
  - `MATCH_RECOGNIZE`（SQL:2016）识别事件序列模式：连续越限、升温骤降、开停机工作流、乱序事件、时间窗约束
  - 模式变量 + 量词（`? * + {n}`）、交替 `|`、`PERMUTE`、导航（`PREV`/`NEXT`/`FIRST`/`LAST`）、聚合、`SUBSET`、`FINAL`/`RUNNING`、`WITHIN` 主动过期
  - 四道闸保证边缘内存有界
- 高可扩展性
  - 提供灵活的函数扩展
