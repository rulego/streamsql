...
// RegisterAggregatorAdapter Registers the aggregator adapter. Usually, manual calls are not required:
// functions.Register automatically calls this function when registering the TypeAggregation function (see registry.go).
// It is only explicitly called when a separate adapter needs to be added to a registered function.
